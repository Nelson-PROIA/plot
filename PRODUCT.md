...
- **Wedge:** PR review (see §1). Single-repo, GitHub-first.
- **Target:** teams with high AI-coding adoption, already shipping fast with
  agents. They feel the understanding gap hardest, today.
...
- **Possible later test** (from the Notion's open question): regulated /
  compliance-heavy industries, where a human attesting to system behavior is
  mandatory and explainability is *bought*, not just liked. Open question, not
  the starting point.
...
1. **Survey, starting now.** Question bank, interview guide, and a tracked
   pipeline (kanban) live in `Nelson | Survey` in the Notion. 15-minute talks,
   starting broad ("how do you use AI"), never pitching. Mostly engineers,
   also EM/product. Goal: verify the pain ranks where we think, learn real
   workflows and tools.
